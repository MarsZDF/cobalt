use cobol_ast::{Program, Statement, ProcedureDivision};
...
impl ControlFlowGraph {
    ...
    fn build_procedure_cfg(&mut self, proc: &ProcedureDivision, entry: &CfgNode) {
        ...
        for (idx, para) in proc.paragraphs.iter().enumerate() {
            ...
            self.paragraph_map.insert(para.node.name.clone(), para_node.clone());
        ...
        for (idx, section) in proc.sections.iter().enumerate() {
            ...
            self.section_map.insert(section.node.name.clone(), section_node.clone());
    ...
    fn build_statement_cfg(&mut self, statements: &[cobol_ast::Spanned<Statement>], start: &CfgNode) {
