use cobol_ast::{Program, DataItem, Statement, ProcedureDivision};
...
impl DeadCodeAnalyzer {
    ...
    fn collect_statement_uses(&self, stmt: &cobol_ast::Spanned<Statement>, used: &mut HashSet<String>) {
        match &stmt.node {
    ...
    fn collect_paragraph_refs_from_stmt(&self, stmt: &cobol_ast::Spanned<Statement>, used: &mut HashSet<String>) {
        match &stmt.node {
            Statement::Perform(perform) => {
                match &perform.node {
                ...
                crate::cfg::CfgNode::Statement(idx) => {
                    if idx < self.program.procedure.node.statements.len() {
                        unreachable.push(UnreachableStatement {
                            statement_type: format!("{:?}", self.program.procedure.node.statements[idx].node),
