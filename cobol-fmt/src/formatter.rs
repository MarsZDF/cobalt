use crate::config::{FormatConfig, IdentifierCase, KeywordCase};
...
impl Formatter {
    ...
    pub fn format(&self, tokens: &[Token], ast: Option<&cobol_ast::Spanned<Program>>) -> String {
        ...
        while i < tokens.len() {
            ...
            if line_length + formatted.len() > self.config.max_line_length
    ...
    fn format_token(&self, token: &Token) -> String {
        match &token.token_type {
            TokenType::Identifier(ref name) => match self.config.identifier_case {
    ...
    fn needs_space_before(&self, prev: Option<&Token>, current: &Token) -> bool {
        ...
        if matches!(prev_type, TokenType::LeftParen)
        ...
        if matches!(
            current.token_type,
            TokenType::Comma | TokenType::Semicolon | TokenType::Period
        ) {
            return false;
        }
    ...
    fn compute_indentation_from_tokens(
        &self,
        tokens: &[Token],
    ) -> std::collections::HashMap<usize, usize> {
