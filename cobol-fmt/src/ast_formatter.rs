use cobol_ast::{Program, DataDivision, DataItem, ProcedureDivision, Statement};
...
impl AstFormatter {
    ...
    fn format_identification_division(&self, id: &cobol_ast::Spanned<cobol_ast::IdentificationDivision>) -> String {
        let mut output = String::new();
    ...
    fn format_environment_division(&self, _env: &cobol_ast::Spanned<cobol_ast::EnvironmentDivision>) -> String {
        let mut output = String::new();
    ...
    fn format_data_item(&self, item: &cobol_ast::Spanned<DataItem>, indent: usize) -> String {
        ...
        if let Some(redefines) = &item.node.redefines {
            output.push_str(&format!(" REDEFINES {}", self.format_identifier(&redefines.node)));
        }
    ...
    fn format_initial_value(&self, value: &cobol_ast::Spanned<cobol_ast::InitialValue>) -> String {
        match &value.node {
            cobol_ast::InitialValue::Literal(lit) => {
                match lit {
            ...
            cobol_ast::InitialValue::FigurativeConstant(fc) => {
                match fc {
        ...
        match &stmt.node {
            ...
            Statement::Move(move_stmt) => {
                ...
                match &move_stmt.node.from.node {
                    cobol_ast::MoveSource::Literal(lit) => {
                        match lit {
                            cobol_ast::Literal::String(s) => output.push_str(&format!(" \"{}\"", s)),
                            cobol_ast::Literal::Numeric(n) => output.push_str(&format!(" {}", n.value)),
                            cobol_ast::Literal::Boolean(b) => output.push_str(&format!(" {}", if *b { "TRUE" } else { "FALSE" })),
